...
def display(x, port=sys.stdout):
    if isa(x, str):
        ...
        print(port.write(to_string(x)))


unsafe_funcs = {     'port?': lambda x:isa(x,file), 'apply':lambda proc,l: proc(*l), 
...
#    _unquote, _cons, _append = builtins['_unquote'], builtins['_cons'], builtins['_append']
    ...
    if not isa(x, list):                 # constant => unchanged
        ...
        require(x, len(x)==4)
        return [y for y in map(local_expand, x)]
    elif x[0] is _set:                   
        require(x, len(x)==3); 
        ...
        return [_lambda, vars, local_expand(exp)]   
    elif x[0] is _quasiquote:            # `x => expand_quasiquote(x)
        require(x, len(x)==2)
        return expand_quasiquote(x[1], builtins)
    elif isa(x[0], Symbol) and x[0] in macro_table:
        return local_expand(macro_table[x[0]](*x[1:])) # (m arg...) 
    else:                                #        => macroexpand if m isa macro
        return [y for y in map(local_expand, x)]            # (f arg...) => expand each


def expand_quasiquote(x, builtins):
    ...
    def generic_eval(x, env=local_env):
        ...
        while True:
            if isa(x, Symbol):       # variable reference
                ...
                (_, test, conseq, alt) = x
                x = (conseq if generic_eval(test, env) else alt)
            elif x[0] is _set:       # (set! var exp)
                (_, var, exp) = x
                env.find(var)[var] = generic_eval(exp, env)
                return None
            elif x[0] is _define:    # (define var exp)
                (_, var, exp) = x
                env[var] = generic_eval(exp, env)
                return None
                ...
                for exp in x[1:-1]:
                    generic_eval(exp, env)
                x = x[-1]
            else:                    # (proc exp*)
                exps = [generic_eval(exp, env) for exp in x]
                proc = exps.pop(0)
...
################ expand
    ...
    def let(*args):
        ...
        return [[_lambda, list(vars)]+ [y for y in map(local_expand, body)]] + [w for w in map(local_expand, vals)]
    ...
    # and_def = [
    # Sym('begin'),
    #  [Sym('define-macro'), Sym('and'),
    #   [Sym('lambda'), Sym('args'),
    #    [Sym('if'), [Sym('null?'), Sym('args')], True,
    #     [Sym('if'), [Sym('='), [Sym('length'), Sym('args')], 1],
    #      [Sym('car'), Sym('args')],
